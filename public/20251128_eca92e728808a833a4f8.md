...
## 原因
[Cloud Storage に適用される IAM 権限  |  Google Cloud Documentation](https://docs.cloud.google.com/storage/docs/access-control/iam-permissions?hl=ja#objects) を確認すると以下の記述があった。
...
## 結論
既存オブジェクトの置換には `storage.objects.create` と `storage.objects.delete` 両方必要。
